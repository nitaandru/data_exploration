...
locatii <- SpatialPointsDataFrame(locatii, coords = locatii[, c(2, 1)]) # converteste in formati geospatial
crs(locatii)<- '+proj=longlat +datum=WGS84 +no_defs' # atribuie o proiectie

extractie <- extract(lunare, locatii, df = T) # extrage datele intr-un nou dataframe
extractie$locatie <- locatii$localitate # insereaza lista cu localitati
extractie$ID <- NULL # indepartam campul ID - nu mai este necesar
names(extractie)[1:12] <- month.abb # redenumit coloanele cu date lunare in numele lunilor

## ploteaza grafice cu valori lunare multianuale
library(reshape2)
library(ggplot2)
...
## ploteaza o harta cu distributia spatiala a valorilor de tg
cordinates <- coordinates(lunare) # extrage valorile coordonatelor
lunare_st <- as.data.frame(lunare) # converteste in dataframe; uneste coordonatele
lunare_st <- cbind(lunare_st, cordinates) # uneste coordonatele de valorile lunare

lunare_st <- na.omit(lunare_st) # exclude valorile lipsa 
lunare_st <- melt(lunare_st, id = c('x', 'y')) # converteste din wide in long format
lunare_st$discrete <- cut(lunare_st$value, breaks  = seq(-25, 40, 5), labels = seq(-25, 35, 5)) # creeaza o scara de valori predefinita

# creaza o paleta de culori (predefinita - vezi https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3)
cols <- rev(RColorBrewer::brewer.pal(11, 'RdYlBu')) # creaza o scara de culori (doar 11 posibile)
cols <- colorRampPalette(cols)(14) # interpoleaza culorile; fa o lista de 14 culori
